...
backup scripts/smoke/voucher_relogin_balance_gate.sh
...
# Update smoke test to send "subtype" instead of invalid Transaction.type
sed -i \
  -e 's/"type": "test_debit_to_zero"/"subtype": "test_debit_to_zero"/g' \
  scripts/smoke/voucher_relogin_balance_gate.sh

echo "[ok] Fixed staff wallet credit/debit to use Transaction.type credit/debit; subtype stored in metadata"
